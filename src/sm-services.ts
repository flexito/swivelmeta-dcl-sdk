...
//  const debug: boolean = false;  // Set to true to enable debug mode
 ...
 export class SwivelMetaServices extends Entity {
     ...
     bInitializeDiscord: boolean = false; // initialize discord callback
             ...
             if ( currentEntity instanceof Entity ) {
                 ...
                 if ( currentComp["component_type"] === "dynamic-media" ){
                     if( currentEntity instanceof DynamicMedia ){
                         ...
                         if ( ( !bLoadOnEnter && currentEntity.video.url === currentMedia ) ||
                         ( !bLoadOnEnter && currentEntity.texture instanceof Texture && temp_currentEntity_texture[temp_currentEntity_texture.length - 1] === temp_currentMedia[temp_currentMedia.length - 1] ) )
                         { continue };
